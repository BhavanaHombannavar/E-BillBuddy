...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $orderItemsTaxRule->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $orderItemsTaxRule->id)]
            )
        ?></li>
...
<div class="orderItemsTaxRules form large-9 medium-8 columns content">
    ...
    <fieldset>
        <legend><?= __('Edit Order Items Tax Rule') ?></legend>
        <?php
            echo $this->Form->control('order_item_id', ['options' => $orderItems]);
            echo $this->Form->control('tax_rule_id', ['options' => $taxRules]);
        ?>
    </fieldset>
